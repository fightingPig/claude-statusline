...
- 修复: install.ps1 `ConvertFrom-Json` 默认 Depth=2 在 PS5.1 下嵌套 JSON 被截断，PS6+ 使用 `-Depth 10`，PS5.1 通过 `ConvertFrom-JsonSafe` 版本兼容（PS5.1 的 `ConvertFrom-Json` 无 `-Depth` 参数）
